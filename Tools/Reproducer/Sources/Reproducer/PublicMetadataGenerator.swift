...
struct PublicMetadataGenerator {
    static func run(options: Options) throws {
        ...
        let resourceFiles = try contentRuleListResourceFiles(in: resourcesDirectoryURL)
        ...
        try writeChecksums(resourceRecords, to: outputDirectoryURL.appendingPathComponent("checksums.sha256"))
    ...
    private static func packageManifest(
        ...
        [
            ...
            "package": [
                "name": packageName,
                "version": releaseVersion(from: generatedAt),
                "license": "GPL-3.0-only",
                "contents": "WebKit Content Rule List JSON resources generated from AdGuard Filters and EasyList."
            ],
            "sourceManifest": "filter-sources.json",
            "sources": sourceManifest["sources"] ?? [],
            "conversion": [
                "inputFormat": "AdGuard/EasyList filter text",
                "outputFormat": "WebKit Content Rule List JSON",
    ...
    private static func contentRuleListResourceFiles(in directoryURL: URL) throws -> [URL] {
    ...
    private static func releaseVersion(from generatedAt: String) -> String {
        "release-" + generatedAt.replacingOccurrences(of: ":", with: "")
    }
}

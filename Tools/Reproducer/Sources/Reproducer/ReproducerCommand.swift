...
struct FilterAssetsReproducer {
    static let contentRuleListTarget: BlockerKitContentRuleListTarget = .iOS17
    ...
    private static func run(arguments: [String]) async throws {
        ...
        case "generate-user-script-artifact":
            try RuntimeArtifactGenerator.run(options: options)
        case "validate-user-script-artifacts":
            try RuntimeArtifactValidator.run(options: options)
        case "compile-content-rules":
            try await compileContentRules(options: options)
        case "validate-content-rules":
            try GeneratedFilterValidator.validate(
                directoryURL: try options.directoryURL(for: "--input-dir")
            )
        case "compress-content-rule-resources":
        ...
        case "validate-compressed-content-rule-resources":
    ...
    private static func convertAdBlock(options: Options) async throws {
        ...
        let runtimeConfigDirectoryURL = options.optionalDirectoryURL(for: "--runtime-config-dir")
        ...
        if let runtimeConfigDirectoryURL {
            try fileManager.createDirectory(at: runtimeConfigDirectoryURL, withIntermediateDirectories: true)
        }
        ...
        let compiler = BlockerKitCompiler(
            options: BlockerKitCompiler.Options(
                ...
                contentRuleListTarget: contentRuleListTarget,
        ...
        let filterTexts = try filterFileURLs.map {
            try String(contentsOf: $0, encoding: .utf8)
        }
        let bundleGroup = try await compileFilterFragments(filterTexts, compiler: compiler)
        guard bundleGroup.bundles.count == filterFileURLs.count else {
            throw ToolError(
                "Grouped BlockerKit compilation returned \(bundleGroup.bundles.count) bundle(s) "
                    + "for \(filterFileURLs.count) input fragment(s)."
            )
        }

        for (filterFileURL, bundle) in zip(filterFileURLs, bundleGroup.bundles) {
            if let runtimeConfigDirectoryURL {
                try RuntimeArtifactGenerator.writeRuntimeConfig(
                    bundle.runtimeConfig,
                    sourceFileURL: filterFileURL,
                    outputPrefix: outputPrefix,
                    outputDirectoryURL: runtimeConfigDirectoryURL
                )
            }
    ...
    @MainActor
    static func compileFilterFragments(
        _ filterTexts: [String],
        compiler: BlockerKitCompiler
    ) async throws -> WKWebViewFilterBundleGroup {
        try await compiler.compile(filterTexts, progress: nil)
    }
    ...
    private static func validatedContentRuleListJSONChunks(
        ...
        for (index, chunk) in chunks.enumerated() {
            let validationIdentifier = "\(identifierBase)-validation-\(index)"
            let partial = try await validContentRuleListJSONChunks(
                from: chunk,
                identifier: validationIdentifier,
                store: store
            )
            if partial.skippedRuleCount == 0 {
                result.chunks.append(contentsOf: partial.chunks)
            } else if let reassembledChunk = try reassembleContentRuleListJSONChunks(partial.chunks) {
                do {
                    let reassembledBundle = try contentRuleListBundle(from: reassembledChunk)
                    guard try await store.compileBlockerKitContentRuleList(
                        identifier: "\(validationIdentifier)-reassembled",
                        from: reassembledBundle
                    ) != nil else {
                        throw ToolError("WKContentRuleListStore returned no rule list.")
                    }
                } catch {
                    throw ToolError(
                        "Reassembled chunk failed WebKit validation after removing isolated invalid rules "
                            + "from \(validationIdentifier): \(error.localizedDescription)"
                    )
                }
                result.chunks.append(reassembledChunk)
            }
    ...
    static func reassembleContentRuleListJSONChunks(_ chunks: [String]) throws -> String? {
        let rules = try chunks.flatMap { try contentRules(in: $0) }
        guard !rules.isEmpty else {
            return nil
        }
        return try contentRuleListJSON(from: rules)
    }
    ...
    private static func printHelp() {
        print(
            ...
            Usage:
              ...
              filter-assets-reproducer convert-adblock --input-dir DIR --output-dir DIR --output-prefix PREFIX [--report-dir DIR] [--runtime-config-dir DIR] [--max-rules-per-chunk COUNT] [--pretty]
              filter-assets-reproducer generate-user-script-artifact --input-dir DIR --output-dir DIR --report-dir DIR --profile-id ID (--input-prefix PREFIX | --input-prefixes PREFIX,...)
              filter-assets-reproducer validate-user-script-artifacts --resources-dir DIR [--checksums-file FILE] [--metadata-dir DIR]
              filter-assets-reproducer validate-content-rules --input-dir DIR
              filter-assets-reproducer compile-content-rules --input-dir DIR --output-dir DIR
              filter-assets-reproducer compress-content-rule-resources --input-dir DIR --output-dir DIR
              filter-assets-reproducer validate-compressed-content-rule-resources --resources-dir DIR [--checksums-file FILE]
              filter-assets-reproducer generate-public-metadata --source-manifest FILE --resources-dir DIR --report-dir DIR --runtime-report-dir DIR --output-dir DIR --package-name NAME [--package-resolved FILE]
            """
        )
    }

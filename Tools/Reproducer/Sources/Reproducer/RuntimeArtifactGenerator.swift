...
struct RuntimeArtifactGenerator {
    ...
    static func run(options: Options) throws {
        let result = try generate(
            ...
            inputPrefix: try options.value(for: "--input-prefix"),
    ...
    static func generate(
        ...
        guard !inputPrefix.isEmpty else {
            throw ToolError("Runtime config input prefix must not be empty.")
        }
        ...
        .filter {
            $0.lastPathComponent.hasPrefix(inputPrefix)
        ...
        guard !inputConfigURLs.isEmpty else {
            throw ToolError(
                "No runtime configs matched prefix \(inputPrefix) in \(inputDirectoryURL.path)."
            )
        }

...
final class RuntimeArtifactGeneratorTests: XCTestCase {
    ...
    func testAllDefaultProfileCombinesEveryDefaultFilterFamily() throws {
        let inputDirectoryURL = temporaryDirectoryURL.appendingPathComponent("all-default-input")
        let outputDirectoryURL = temporaryDirectoryURL.appendingPathComponent("all-default-output")
        let reportDirectoryURL = temporaryDirectoryURL.appendingPathComponent("all-default-reports")
        try FileManager.default.createDirectory(at: inputDirectoryURL, withIntermediateDirectories: true)

        let configs = [
            ("adguard_base_base.runtime-config.json", "adguard-base", "base.example"),
            ("adguard_japanese_specific.runtime-config.json", "adguard-japanese", "jp.example"),
            ("easylist_easylist_general_hide.runtime-config.json", "easylist-general", "example.com"),
            ("easylist_easylist_allowlist.runtime-config.json", "easylist-allowlist", "x.com"),
            ("easylist_adult_adult.runtime-config.json", "easylist-adult", "adult.example")
        ]
        for (fileName, marker, domain) in configs {
            try writeRuntimeConfig(
                runtimeConfig(marker: marker, domain: domain),
                to: inputDirectoryURL.appendingPathComponent(fileName)
            )
        }
        ...
        XCTAssertEqual(result.inputConfigFileNames, configs.map(\.0).sorted())
        XCTAssertEqual(result.ruleCounts.total, configs.count * 4)
        let source = try String(
            contentsOf: outputDirectoryURL.appendingPathComponent(result.artifactFileName),
            encoding: .utf8
        )
        XCTAssertTrue(source.contains(".easylist-general"))
        XCTAssertTrue(source.contains("x.com"))
        XCTAssertEqual(try decodeManifest(in: outputDirectoryURL).profiles.map(\.id), ["all-default"])
    }

    func testValidatorRejectsRuntimeConfigWithRulesMissingFromEveryProfile() throws {
        let metadataDirectoryURL = temporaryDirectoryURL.appendingPathComponent("metadata")
        let resourcesDirectoryURL = metadataDirectoryURL.appendingPathComponent(
            "Sources/FilterAssets/Resources/AdBlock"
        )
        let inputDirectoryURL = temporaryDirectoryURL.appendingPathComponent("completeness-input")
        let runtimeReportDirectoryURL = metadataDirectoryURL.appendingPathComponent("reports/runtime")
        let blockerKitReportDirectoryURL = metadataDirectoryURL.appendingPathComponent("reports/blockerkit")
        try FileManager.default.createDirectory(at: inputDirectoryURL, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(
            at: blockerKitReportDirectoryURL,
            withIntermediateDirectories: true
        )
        try writeRuntimeConfig(
            runtimeConfig(marker: "japanese"),
            to: inputDirectoryURL.appendingPathComponent(
                "adguard_japanese_specific.runtime-config.json"
            )
        )
        _ = try RuntimeArtifactGenerator.generate(
            profileID: "all-default",
            inputPrefixes: ["adguard_japanese_"],
            inputDirectoryURL: inputDirectoryURL,
            outputDirectoryURL: resourcesDirectoryURL,
            reportDirectoryURL: runtimeReportDirectoryURL
        )
        try writeDetailedConversionReport(
            userScriptRuleCount: 4,
            generatedContentRuleCount: 1,
            to: blockerKitReportDirectoryURL.appendingPathComponent(
                "adguard_japanese_specific.json"
            )
        )
        ...
        try writeDetailedConversionReport(
            userScriptRuleCount: 13_641,
            generatedContentRuleCount: 0,
            to: blockerKitReportDirectoryURL.appendingPathComponent(
                "easylist_easylist_general_hide.json"
            )
        )
        XCTAssertThrowsError(
            try RuntimeArtifactValidator.validate(
            ...
            XCTAssertTrue(
                error.localizedDescription.contains(
                    "easylist_easylist_general_hide.runtime-config.json"
                )
            )
        }
    }
    ...
    private func writeDetailedConversionReport(
        userScriptRuleCount: Int,
        generatedContentRuleCount: Int,
        to url: URL
    ) throws {
        let object: [String: Any] = [
            "report": [
                "statistics": [
                    "userScriptRuleCount": userScriptRuleCount,
                    "generatedContentRuleCount": generatedContentRuleCount
                ]
            ]
        ]

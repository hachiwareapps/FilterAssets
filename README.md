...
`BlockerKitUserScript-adguard-japanese.js` は `adguard_japanese` の runtime config を入力ファイル名順に統合した1本の script で、注入条件は `documentStart`、全 frame、page content world とする。
...
`verify.sh` は再生成物を公開中の `checksums.sha256` と照合し、user script manifest の schema、対応ファイル、checksum も検証する。

...
RUNTIME_CONFIG_DIR="$WORK_DIR/runtime-configs"
RUNTIME_REPORT_DIR="$WORK_DIR/runtime-reports"
...
mkdir -p "$JSON_DIR" "$REPORT_DIR" "$RUNTIME_CONFIG_DIR" "$RUNTIME_REPORT_DIR" "$SOURCE_DIR" "$RESOURCE_DIR"
...
while IFS=$'\t' read -r source_id archive_url included_directory output_prefix; do
  ...
  swift run --disable-sandbox --package-path "$REPRODUCER_DIR" filter-assets-reproducer convert-adblock \
    ...
    --runtime-config-dir "$RUNTIME_CONFIG_DIR" \
    --max-rules-per-chunk "$MAX_RULES_PER_CHUNK"
done < "$PLAN_FILE"

swift run --disable-sandbox --package-path "$REPRODUCER_DIR" filter-assets-reproducer generate-user-script-artifact \
  --input-dir "$RUNTIME_CONFIG_DIR" \
  --output-dir "$RESOURCE_DIR" \
  --report-dir "$RUNTIME_REPORT_DIR" \
  --profile-id all-default \
  --input-prefixes adguard_base_,adguard_japanese_,easylist_easylist_,easylist_adult_
...
swift run --disable-sandbox --package-path "$REPRODUCER_DIR" filter-assets-reproducer generate-public-metadata \
  ...
  --report-dir "$REPORT_DIR" \
  --runtime-report-dir "$RUNTIME_REPORT_DIR" \
